"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
/*
 * Copyright (c) 2020 SAP SE or an SAP affiliate company. All rights reserved.
 *
 * This is a generated file powered by the SAP Cloud SDK for JavaScript.
 */
var core_1 = require("@sap-cloud-sdk/core");
/**
 * @deprecated since v1.6.0. Use [[ThemeColor.build]] instead.
 */
function createThemeColor(json) {
    return ThemeColor.build(json);
}
exports.createThemeColor = createThemeColor;
/**
 * ThemeColorField
 * @typeparam EntityT - Type of the entity the complex type field belongs to.
 */
var ThemeColorField = /** @class */ (function (_super) {
    __extends(ThemeColorField, _super);
    function ThemeColorField() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        /**
         * Representation of the [[ThemeColor.value]] property for query construction.
         * Use to reference this property in query operations such as 'filter' in the fluent request API.
         */
        _this.value = new core_1.ComplexTypeStringPropertyField('value', _this, 'Edm.String');
        return _this;
    }
    return ThemeColorField;
}(core_1.ComplexTypeField));
exports.ThemeColorField = ThemeColorField;
var ThemeColor;
(function (ThemeColor) {
    function build(json) {
        return core_1.createComplexType(json, {
            value: function (value) { return ({ value: core_1.edmToTs(value, 'Edm.String') }); }
        });
    }
    ThemeColor.build = build;
})(ThemeColor = exports.ThemeColor || (exports.ThemeColor = {}));
//# sourceMappingURL=ThemeColor.js.map