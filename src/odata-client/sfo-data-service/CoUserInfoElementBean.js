"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
/*
 * Copyright (c) 2020 SAP SE or an SAP affiliate company. All rights reserved.
 *
 * This is a generated file powered by the SAP Cloud SDK for JavaScript.
 */
var core_1 = require("@sap-cloud-sdk/core");
/**
 * @deprecated since v1.6.0. Use [[CoUserInfoElementBean.build]] instead.
 */
function createCoUserInfoElementBean(json) {
    return CoUserInfoElementBean.build(json);
}
exports.createCoUserInfoElementBean = createCoUserInfoElementBean;
/**
 * CoUserInfoElementBeanField
 * @typeparam EntityT - Type of the entity the complex type field belongs to.
 */
var CoUserInfoElementBeanField = /** @class */ (function (_super) {
    __extends(CoUserInfoElementBeanField, _super);
    function CoUserInfoElementBeanField() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        /**
         * Representation of the [[CoUserInfoElementBean.id]] property for query construction.
         * Use to reference this property in query operations such as 'filter' in the fluent request API.
         */
        _this.id = new core_1.ComplexTypeStringPropertyField('id', _this, 'Edm.String');
        /**
         * Representation of the [[CoUserInfoElementBean.label]] property for query construction.
         * Use to reference this property in query operations such as 'filter' in the fluent request API.
         */
        _this.label = new core_1.ComplexTypeStringPropertyField('label', _this, 'Edm.String');
        /**
         * Representation of the [[CoUserInfoElementBean.type]] property for query construction.
         * Use to reference this property in query operations such as 'filter' in the fluent request API.
         */
        _this.type = new core_1.ComplexTypeStringPropertyField('type', _this, 'Edm.String');
        /**
         * Representation of the [[CoUserInfoElementBean.value]] property for query construction.
         * Use to reference this property in query operations such as 'filter' in the fluent request API.
         */
        _this.value = new core_1.ComplexTypeStringPropertyField('value', _this, 'Edm.String');
        return _this;
    }
    return CoUserInfoElementBeanField;
}(core_1.ComplexTypeField));
exports.CoUserInfoElementBeanField = CoUserInfoElementBeanField;
var CoUserInfoElementBean;
(function (CoUserInfoElementBean) {
    function build(json) {
        return core_1.createComplexType(json, {
            id: function (id) { return ({ id: core_1.edmToTs(id, 'Edm.String') }); },
            label: function (label) { return ({ label: core_1.edmToTs(label, 'Edm.String') }); },
            type: function (type) { return ({ type: core_1.edmToTs(type, 'Edm.String') }); },
            value: function (value) { return ({ value: core_1.edmToTs(value, 'Edm.String') }); }
        });
    }
    CoUserInfoElementBean.build = build;
})(CoUserInfoElementBean = exports.CoUserInfoElementBean || (exports.CoUserInfoElementBean = {}));
//# sourceMappingURL=CoUserInfoElementBean.js.map